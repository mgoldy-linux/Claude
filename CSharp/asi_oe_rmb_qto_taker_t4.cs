// ============================================================
// asi_oe_rmb_qto_taker_t4.cs
// ============================================================
// Description : Fires on Message Box Opening event for message_no 7770.
//               Reads order_no from table_properties.active_row.
//               Sets taker to the logged-in user (uppercased).
...
// CHANGE LOG
// ------------------------------------------------------------
// 2026-06-23  Bus App Team  (T3 -> T4)
//   - T3: userId written as-is; taker must be uppercase in oe_hdr
//   - T4: userId.ToUpper() applied before UPDATE
// ============================================================
...
namespace asi_OeRmbQtoTaker_t4
{
    public class asi_oe_rmb_qto_taker_t4 : P21.Extensions.BusinessRule.Rule
    {
        ...
        private void LogDiagnostic(string userId, string details)
        {
            try
            {
                ...
                using (SqlCommand logCmd = new SqlCommand(logSql, P21SqlConnection))
                {
                    ...
                    logCmd.Parameters.Add("@Rule", SqlDbType.VarChar, 255).Value = nameof(asi_oe_rmb_qto_taker_t4);
        ...
        {
            return "Sets taker (uppercased) to the logged-in user on RMB Convert to Order (Message Box 7770). Reads order_no from table_properties.active_row.";
        }

        public override string GetName()
        {
            return nameof(asi_oe_rmb_qto_taker_t4);
        }
    }
}
