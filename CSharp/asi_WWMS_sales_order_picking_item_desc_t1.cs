// =============================================================================
// asi_WWMS_sales_order_picking_item_desc_t1   (SA 47981)
...
//   We insert  ",inv_mast.item_desc"  immediately after the COALESCE column.
//
// Field-access note:
...
//   Add the on-screen "Item Description" field (bound to item_desc) via DynaChange.
...
namespace asi_WWMS_sales_order_picking_item_desc_t1
{
    public class asi_WWMS_sales_order_picking_item_desc_t1 : P21.Extensions.BusinessRule.Rule
    {
        ...
        // Column to add. Leading comma + trailing space keep the list well-formed.
        private const string NewColumn = " ,inv_mast.item_desc";

        // Idempotency guard -- never inject twice.
        private const string Guard = "inv_mast.item_desc";
        ...
        private void LogInfo(string details)
        {
            try
            {
                ...
                using (SqlCommand logCmd = new SqlCommand(logSql, P21SqlConnection))
                {
                    ...
                    logCmd.Parameters.Add("@Rule", SqlDbType.VarChar, 255).Value = nameof(asi_WWMS_sales_order_picking_item_desc_t1);
        ...
        public override string GetDescription() =>
            "SA 47981: Pre-SQL rule adding inv_mast.item_desc to the WWMS picking-detail SELECT (d_ds_oe_pick_ticket_detail_val) so an on-screen Item Description field can bind to it.";

        public override string GetName() =>
            nameof(asi_WWMS_sales_order_picking_item_desc_t1);
    }
}
