// ============================================================
// asi_oe_qto_taker_t6.cs
// ============================================================
// Description : DIAGNOSTIC — dumps Data.Set for the Window Opened event to
//               capture the exact window_name and window_title strings when
//               the Quote to Order window opens. Use the output to filter
//               the production rule to that window only.
// Event       : Window Opened
// Field Selector: WindowOpenedData > window_name, window_title (both checked)
// ============================================================
// CHANGE LOG
// ------------------------------------------------------------
// 2026-06-22  Bus App Team  (T5 → T6)
//   - T5: attempting Window Opened event approach to get window name
//   - T6: diagnostic — dump Data.Set to find exact window_name / window_title strings
// ============================================================
...
namespace asi_OeQtoTaker_t6
{
    public class asi_oe_qto_taker_t6 : P21.Extensions.BusinessRule.Rule
    {
        public override RuleResult Execute()
        {
            ...
            {
                ruleResult.Success = true;
                ruleResult.Message = "T6 diagnostic error: " + ex.Message;
            }
        ...
        private void LogDiagnostic(string details)
        {
            try
            {
                ...
                using (SqlCommand logCmd = new SqlCommand(logSql, P21SqlConnection))
                {
                    ...
                    logCmd.Parameters.Add("@Rule", SqlDbType.VarChar, 255).Value = nameof(asi_oe_qto_taker_t6);
        ...
        {
            return "DIAGNOSTIC: Captures window_name and window_title from the Window Opened event to identify the QTO conversion window.";
        }

        public override string GetName()
        {
            return nameof(asi_oe_qto_taker_t6);
        }
    }
}
