...
//   t9  - Renamed to _t9_oe_order_sales_loc_id_up
//         Fix: @locationId passes DBNull when defaultBranch is empty so IS NULL
//         contracts are matched correctly
//         Fix: explicit P21SqlConnection open check before executing queries
// =============================================================================
...
namespace _t9_oe_order_sales_loc_id_up
{
    public class _t9_oe_order_sales_loc_id_up : P21.Extensions.BusinessRule.Rule
    {
        public override RuleResult Execute()
        {
            ...
            try
            {
                ...
                // Customer checks — overdue invoices, contacts, multiple contracts
                if (!string.IsNullOrWhiteSpace(customerId))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand(contractQuery, P21SqlConnection))
                    {
                        ...
                        cmd.Parameters.Add("@customerId", SqlDbType.VarChar, 50).Value = customerId;
                        cmd.Parameters.Add("@shipToId",   SqlDbType.VarChar, 50).Value = shipToId;
                        cmd.Parameters.Add("@locationId", SqlDbType.VarChar, 50).Value =
                            string.IsNullOrWhiteSpace(defaultBranch) ? (object)DBNull.Value : defaultBranch;
            ...
            {
                ruleResult.Message = $"[_t9_oe_order_sales_loc_id_up] Error: ship_to_id='{shipToId}' customer_id='{customerId}' — {ex.Message}";
            }
