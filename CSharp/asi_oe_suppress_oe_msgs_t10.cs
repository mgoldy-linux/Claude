...
//   t10 - Fixed cast error: use Convert.ToInt32 for message_no instead of Field<int>
// =============================================================================
...
namespace asi_oe_suppress_oe_msgs
{
    public class asi_oe_suppress_oe_msgs : P21.Extensions.BusinessRule.Rule
    {
        public override RuleResult Execute()
        {
            ...
            int messageNo    = Convert.ToInt32(msgRow["message_no"]);
            ...
            {
                string messageTitle = msgRow["message_title"] == DBNull.Value ? null : msgRow["message_title"].ToString();
                string methodName   = msgRow["method_name"]   == DBNull.Value ? null : msgRow["method_name"].ToString();
                string userText     = msgRow["user_text"]     == DBNull.Value ? null : msgRow["user_text"].ToString();
        ...
        private void LogMessage(int messageNo, string messageTitle, string methodName, string userText)
        {
            ...
            using (SqlCommand cmd = new SqlCommand("dbo.usp_suppress_messages_p21_ud", P21SqlConnection))
            {
                ...
                cmd.Parameters.AddWithValue("@message_no",    messageNo);
                cmd.Parameters.AddWithValue("@message_title", (object)messageTitle ?? DBNull.Value);
                cmd.Parameters.AddWithValue("@method_name",   (object)methodName   ?? DBNull.Value);
                cmd.Parameters.AddWithValue("@user_text",     (object)userText     ?? DBNull.Value);
                cmd.ExecuteNonQuery();
            }
        }
