/*==============================================================================
  06 — Harden p21_view_alert_oe_OrderEntry against the NULL-token collapse bug
       in p21_sp_alert_generation (2026-08-11)

  p21_sp_alert_generation builds each alert's subject/header/footer/line-item
  text by chaining SET @PX = REPLACE(@PX, '<column>', @value) once per token
  registered for the alert's TYPE (all OE alerts share the same ~90-token pool,
  regardless of which tokens that specific alert's own template references).
  REPLACE() returns NULL if ANY argument is NULL — confirmed empirically:
    REPLACE('Order for <customer_name>', '<zzz_not_present>', NULL) --> NULL
  So a single NULL token value can blank an entire email section, even one
  the alert's own template never mentions. See [[feedback_p21_alerts]].

  Audited every column in this view that's registered as an OE (alert_type_uid
  12) token with available_areas hitting subject/header/footer/line-item
  (bits 1/2/4/8). Found 6 unwrapped against the pool most of the view already
  guards (leftover from before the 2026-07 Low Margin Alert hardening pass):
    order_profit_percentage, extended_price, total_amount,
    line_item_profit_percentage, fill_quantity, fill_extended_price

  MEASURED, not assumed: checked all 6 against 90 days of real order lines in
  Play/Training/BusinessRules (~1.5M rows) with filters matching the view's
  own WHERE clause exactly (delete_flag='N', completed<>'T', parent_oe_line_uid=0)
  — zero actual NULLs or divide-by-zero conditions found in any of them today.
  This change is a no-op on all existing data (proven via the EXCEPT-equivalent
  check below) — it's insurance against a future edge case (new ECOMM path,
  data migration, manual insert), not a fix for an observed failure.

  Does NOT explain the 2026-08-11 blank email from order 6073955 (P21Training,
  uid 102 "Test Verify Alerts") — that order's own fields were all populated.
  That incident's real cause is still open; see [[feedback_p21_alerts]].

  Run once per environment: P21Play, P21Training, P21BusinessRules (identical
  view definition confirmed byte-for-byte across all three, 2026-08-11).
  Do NOT run against Prod without first diffing Prod's view — its Low Margin
  Alert columns were added via a separate PROD-suffixed script and may not be
  byte-identical to Play's; see [[feedback_prod_view_drift_before_srd_deploy]].
==============================================================================*/

-- USE <target database>;  -- P21Play | P21Training | P21BusinessRules
GO
