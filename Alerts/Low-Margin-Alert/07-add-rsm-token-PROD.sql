/*==============================================================================
  ...
  Target : P21 (PROD)
  Author : Mark Goldyn / Claude          Date: 2026-08-28
  Depends on: PROD's existing p21_view_alert_oe_OrderEntry (from
  01-alter-view-add-margin-columns-PROD.sql / 02-register-tokens-PROD.sql,
  deployed 2026-07-31, still row_status_flag = 705 INACTIVE on 104/105).

  Mirrors 07-add-rsm-token.sql (Play, 2026-08-27), now proven live: the RSM
  token resolves correctly (verified against 6+ real reps in Play) and Evan
  signed off on it (8/27 reply) plus confirmed via a real test send to Tyler
  Priewe (8/28) that it works end-to-end. This script ports ONLY that change
  to Prod — the view + token registration. It does NOT touch alert_recipient
  on Prod uid 104/105 (still deliberately inactive/705), and does NOT include
  the 2026-08-11 NULL-token hardening (script 06) — that is a SEPARATE,
  already-flagged, not-yet-approved Prod change (see
  feedback_prod_view_drift_before_srd_deploy.md /
  project_2026_07_13_low_margin_alert.md, "2026-08-11" section). Confirmed via
  a full diff against Play's current view (2026-08-28) that this is the ONLY
  drift being closed here — Prod's baseline is otherwise unchanged since the
  7/31 deploy.

  Same RSM join pattern as Play — P21's own stock pattern from
  p21_view_alert_cr_Opportunities:
      LEFT JOIN contacts contacts_salesrep_manager
        ON contacts_salesrep_manager.id = contacts_salesrep.sales_manager_id
  contacts.sales_manager_id is a native P21 column — NOT kb_-owned. See
  07-add-rsm-token.sql (Play) for the full kb_ flag / data-quality notes;
  same reasoning applies here, not repeated.

  ⚠ Adding <rsm_email> to alert_recipient on Prod 104/105 is a SEPARATE step,
  not part of this script — that only matters once 104/105 go active (targeted
  9/2 per Evan), and needs the recipient-scope decision (Team/Purchasing/both,
  To/CC) plus Evan-on-Team / Jere Butler-on-Purchasing / BCC-to-self, all
  still pending.
==============================================================================*/
...
SELECT	oe_hdr.order_no 'view_key'
		...
		-- mg add 2026-08-28: RSM token (feedback via Evan, proven in Play 8/27, tested live 8/28) —
		-- salesrep's manager email, same pattern p21_view_alert_cr_Opportunities uses for salesrep_manager_email
...
-- mg add 2026-08-28: RSM token — same join P21 stock uses in p21_view_alert_cr_Opportunities
