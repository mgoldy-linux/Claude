...
| 2026-07-27 | 90 Day Usage Report — blank reports on several branches (no ticket, Jami Klitzman) | 🔵 In progress | **Root cause narrowed.** Subscription 241 (fixed 7/23) relapsed to blank by its 7/26 Sunday run — proof the original "rebuild the 328-value list" fix doesn't hold. Dug deeper: report's `mfg` parameter is deployed as `MultiValue=False` (since the 2026-06-01 redeploy), but subscriptions still store 300+ literal entries under that name — a type mismatch. `mfg`'s real default resolves to `NULL`, and the query already handles `NULL` as "no filter," so the durable fix is switching every location's `mfg` from a frozen "Enter value" list to "Use Default Value," not rebuilding the list again. Confirmed via SOAP API: 46 of 49 subscriptions are currently on the static list; only 100/103/241 are on default (all three edited by the user that same morning while testing). Built + dry-run-verified `Update-NinetyDayUsageSubscriptions-MfgUseDefault.ps1` (scoped to the 44 per-location subs only, excludes Karndean/Lotte/etc. which intentionally filter to one manufacturer) — **not yet applied, user will run `-Apply` themselves.** Also built `Export-90DayUsageSubscriptionParameters.ps1` (`C:\PowerShell-Scripts\Create-Excel\`) as a full per-location parameter/source-of-value dump, doubling as the after-fix verification tool. Confirmed each subscription has its own private schedule (safe to rerun individually). Drafted (not sent) a reply to Jami on the live thread explaining the fix and asking whether she wants a forced rerun tonight or to wait for Sunday. Next: (1) user applies the fix, (2) Jami answers rerun-tonight vs. Sunday, (3) 5 newly-reported branches (Naperville, St. Cloud, Omaha, Wixom, Ft Wayne) not yet individually checked — assumed covered by the same fix | `project_2026_07_23_ssrs_90day_usage_blank.md` |
