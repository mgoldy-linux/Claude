...
| 2026-07-21 | SA-47981 WWMS Pick Item Desc | ⏸ Parked (needs Epicor) | **Data path solved, display not reachable via DynaChange on the RF picking screen.** Root causes found: screen-only column's real name is `ufc_p21soc_wwmsitemdesc` (auto-prefix, not case); item vs. screen field live on different datawindows (`d_dw_oe_pick_ticket_detail_freight` vs `d_dw_rf_shipping_bin`); no rule type can render a per-row custom column (Converter output only goes to its trigger field + only Field Edit/Save/Window Opening timing; On Event has no pick-line event). Converter logs `SET ...` but nothing renders. Next: **ask Epicor** for the supported RF-screen customization (Visual Web Rule?). Non-tech ticket update written. Also fixed a Class Name trap (validations vs detail_val binding) | `project_2026_06_25_sa47981_wwms_pick_item_desc.md` |
