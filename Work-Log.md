...
| 2026-07-28 | 90 Day Usage Report — blank reports on several branches (no ticket, Jami Klitzman) | 🔵 In progress | **Fix confirmed applied:** `dbo.Subscriptions.ModifiedDate` shows a batch update (14:49-14:50 on 7/27) across all 44 per-location subscriptions — `Update-NinetyDayUsageSubscriptions-MfgUseDefault.ps1 -Apply` was run. **Jami answered the open question: she wants all locations sent tonight, one-time.** Built `PowerShell-Scripts\Invoke-NinetyDayUsageSubscriptions-FireTonight.ps1` — pulls the 44 per-location Subscription/Schedule pairs live and fires each via `FireEvent("TimedSubscription", ScheduleID)`, same as a real schedule trigger (real email, real recipients), without touching the Sunday schedule itself. Default is dry run; `-Apply` fires for real. Recommended running it this evening, before the `asdwdb01` 2:05 AM nightly restore window. **Not yet run — awaiting the user to execute it tonight.** Next: (1) user runs `-Apply` this evening, (2) check `LastStatus`/`ExecutionLog3` after to confirm all 44 rendered non-empty, (3) 5 newly-reported branches (Naperville, St. Cloud, Omaha, Wixom, Ft Wayne) still not individually checked — assumed covered by the same fix but unverified | `project_2026_07_23_ssrs_90day_usage_blank.md` |
