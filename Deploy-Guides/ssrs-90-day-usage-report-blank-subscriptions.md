...
- `PowerShell-Scripts\Update-NinetyDayUsageSubscriptions-MfgUseDefault.ps1` — the durable fix: switches `mfg` from a static "Enter value" list to "Use Default Value" for all 44 per-location subscriptions. **Dry-run tested, NOT yet applied.**
...
Problem 1's fix (2026-07-23) is superseded — it did not hold. Problem 2 (intermittent bug) is very likely explained by problem 3, but that connection is not proven. Problem 3's fix is built and dry-run tested, **not yet applied**.
...
**2026-07-27 fix (current, not yet applied):**
...
- **2026-07-27 fix, once applied:** re-run `Export-90DayUsageSubscriptionParameters.ps1` — both it and the update script hit the same live SOAP endpoint with no caching, so the `mfg_Source` column should immediately read "Use Default Value" for the 41 changed subscriptions. Real-world verification is external: Jami / branch managers confirming reports come through with data, either off a forced rerun tonight or the normal Sunday schedule — **still pending her answer**.
...
- **Apply the 2026-07-27 fix and verify** — not yet done as of this writing.
- **Confirm with Jami: rerun all 44 tonight for validation, or wait for the normal Sunday schedule?** Draft reply sent to her Drafts-adjacent live thread, not yet answered. Each subscription has its own private `Schedule` (confirmed via `dbo.ReportSchedule`/`dbo.Schedule`, 1 subscription per `ScheduleID`), so a forced rerun of one location cannot spill into others if that path is chosen.
- **5 newly-reported branches not yet individually checked:** Naperville, St. Cloud, Omaha, Wixom, Ft Wayne — assumed covered by the same across-the-board `mfg` fix since it's not location-specific, but that's an assumption.
