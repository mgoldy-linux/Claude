...
- SQL extracted from the `.srd` runs clean on Play: **2,142 rows**, all 19 columns bind, every taker
  returned is Customer Service / CS Manager. (Play shows only 3 CSR takers — it's a stale refresh.)
- Prod expected volume: **6,835 open order lines** across all 14 CSRs.
- In the portal: the **Taker** column is the first column; every value should be a CSR or CS Manager,
  and no `E COMM` / `E Store` / `Shag Tools` rows should appear.
