...
| *(none)* | [90 Day Usage Report — blank subscriptions (Jami Klitzman)](ssrs-90-day-usage-report-blank-subscriptions.md) | 🔵 **2026-07-23 fix superseded** — subscription 241 relapsed by its next Sunday run. Real root cause: report's `mfg` parameter redeployed as single-value 2026-06-01, but subscriptions still store 300+ literal entries under it. Durable fix (switch to "Use Default Value" for all 44 locations) built and dry-run tested, **not yet applied**. 5 new branches reported (Naperville, St. Cloud, Omaha, Wixom, Ft Wayne). Awaiting: user to apply fix; Jami's answer on rerun-tonight vs. Sunday |
