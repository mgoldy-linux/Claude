...
select distinct a.phys_address1[Address],Left(a.phys_postal_code,6)[ZipCode],gc.direct_phone,[Location ID],[Customer ID],[Customer ID][ShipTo ID],[Contact ID]
from getcontacts gc
join address a
on gc.[Customer ID] = a.id
where a.phys_postal_code like '%[A-Z]%'
order by ZipCode