...
FROM            P21.dbo.p21_view_invoice_line AS invoice_line LEFT OUTER JOIN
                         ...
                         P21.dbo.territory_x_customer AS txc ON customer.customer_id = txc.customer_id LEFT OUTER JOIN
...
WHERE        (invoice_hdr.approved = 'Y') AND (invoice_hdr.invoice_class <> 'FINANCE') AND (invoice_hdr.invoice_adjustment_type NOT IN ('B', 'T', 'X', 'R', 'W', 'A', 'P', 'Z', 'Y')) AND (invoice_hdr.consolidated <> 'C') AND 
                         ...
                         (invoice_line.invoice_line_type NOT IN (3)) AND (invoice_hdr.original_document_type <> 'T') AND (invoice_line.item_id NOT IN ('DOWNPAYMENT', 'PREPAYMENT')) AND (invoice_line.qty_shipped > 0.0000 OR
                         invoice_line.qty_shipped < 0.0000) AND (invoice_line.item_id <> 'ADVANCE BILL AMOUNT') AND (oe_line.lot_bill = 'Y') AND (invoice_line.invoice_line_uid_parent = 0) AND (invoice_line.invoice_line_type = 4)-- and invoice_hdr.invoice_no = '3474591'