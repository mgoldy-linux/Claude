-- 52671
use P21;
select  distinct item_id, item_desc, class_id1,class_id2,class_id3, class_id5, default_purchase_disc_group, '9999'[supplier_id],ist.country_of_origin, schedule_b_number,unit_of_measure,unit_size[UNIT SIZE],m.default_selling_unit[default_sales_unit],
case
when unit_of_measure = 'EA' then 'Y'
else 'N'
...
where default_purchase_disc_group = 'Tritan' and m.delete_flag = 'N' -- and item_id = '2101052602' -- = '2101052504'
order by item_id

/*
select top 5*
from dbo.price_family
where price_family_uid = 43

select default_price_family_uid,de
from inv_mast 
where item_id = '2101052602'

select *
from inventory_supplier
where inv_mast_uid = 52671

select *
from dbo.inv_mast_trade
where inv_mast_uid = 52671
...
where inventory_supplier_uid = 75427
