/*
	I didn't create this view
	02/28/2020 add requested_date per Ralph, tested in P21Play
	08/28/2020 add ship2_state, oe_hdr.shipping_route_uid
	12/09/2020 swap the order of dbo.oe_pick_ticket.tracking_no,dbo.p21_view_clippership_return_10004.tracking_no
*/
use P21Play;
go
/*
if OBJECT_ID ('aaa_missing_pick_tickets', 'V') is not null
drop view aaa_missing_pick_tickets;
go
create view [dbo].[aaa_missing_pick_tickets] AS
*/
SELECT        dbo.oe_pick_ticket.pick_ticket_no,oe_hdr.shipping_route_uid[Route],dbo.oe_pick_ticket.print_date,dbo.oe_hdr.requested_date, COALESCE (dbo.p21_view_clippership_return_10004.carrier_name, dbo.address.name) AS c_carrier, 
                         COALESCE ( dbo.oe_pick_ticket.tracking_no,dbo.p21_view_clippership_return_10004.tracking_no,ptshipment.shipment_id) AS c_tracking_no, COALESCE (dbo.p21_view_clippership_return_10004.total_charge, 
                         ...
                         THEN 'R' WHEN oe_pick_ticket_ups.sent_to_roadnet_flag = 'P' THEN 'P' ELSE 'N' END AS roadnet_status, dbo.freight_code.freight_desc, dbo.oe_hdr.ship2_name,dbo.oe_hdr.ship2_state[Ship_2_State],dbo.oe_hdr.order_date, dbo.oe_hdr.location_id, 
                         dbo.oe_hdr.taker, dbo.oe_hdr.third_party_billing_flag, dbo.oe_hdr.approved, dbo.oe_hdr.delete_flag AS Expr1, dbo.oe_hdr.projected_order, dbo.oe_hdr.cancel_flag, ptshipment.shipment_desc, ptshipment.shipment_id, 
                         dbo.oe_pick_ticket.instructions
...
WHERE (NOT (dbo.oe_pick_ticket.delete_flag = 'Y')) AND (NOT (dbo.oe_pick_ticket.auxiliary = 'Y')) OR (NOT (dbo.oe_pick_ticket.pick_and_hold = 'Y')) -- and dbo.oe_pick_ticket.pick_ticket_no = 2093968