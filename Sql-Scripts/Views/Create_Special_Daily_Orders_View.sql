...
if OBJECT_ID ('Special_Daily_Orders_VW', 'V') is not null
drop view Special_Daily_Orders_VW;
go

create view [dbo].[Special_Daily_Orders_VW] AS
...
(
	...
	case when ship2_zip is null then 'x607x' else ship2_zip end,
	case when ship2_city is null then 'No_City' else ship2_city end,
	case when ship2_state is null then 'No_State' else ship2_state end
	from oe_hdr h
	join contacts c
	on h.contact_id = c.id
	/*Manually enter data & time'*/
	where order_date between '2023-11-08 18:00:00.00'  and '2023-11-09 18:00:00.00' and cancel_flag = 'N' and projected_order = 'N'
),
...
(
	select case 
		when cid.salesrep_id  = 1004 then 'PTI HOUSE CORPORATE'
		when cid.salesrep_id  = 1005 then 'LMS House Account'
		when cid.salesrep_id  = 1006 then 'IPTCI HOUSE CORPORATE'
		when cid.salesrep_id in (1004,1013,1019,1028,1030,10653,18353,40391) then 'George Dib'
		when cid.salesrep_id in (1020,1022,1023,1032,1033,1038,1039,1040,10654,10659,34445) then  'Ryan Linke'
		when cid.salesrep_id in (1017,1021,1025,1026,1029,1035,34446,36840) then 'Scott Kuhn'
		when cid.salesrep_id in (40935,40936,40937,40938,40939,40942) then 'Jeremy Bloom'
		else 'Unassigned' end,
...
select srm[SalesManager],salesrep_id,name[CompanyName],customer_part_number,item_desc,product_group_id,qty_ordered,extended_price,ship2_CompanyName,order_no,convert(varchar(2),(datepart(MONTH,order_date)))+ '/' + convert(varchar(2),(datepart(day,order_date)))  + '/'+ convert(varchar(4),(datepart(YEAR,order_date)))[order_date],po_no,approved,customer_id,legacy_id,SalesRepName,taker,SalesBranchID,ShipFromID,TelNo,CustName,Addr,ZipCode,City,SState,unit_price
...
grant select on object::Special_Daily_Orders_VW to p21_application_role
grant select on object::Special_Daily_Orders_VW to PxxiUser
grant select on object::Special_Daily_Orders_VW to [PTIDOM\P21Users]
