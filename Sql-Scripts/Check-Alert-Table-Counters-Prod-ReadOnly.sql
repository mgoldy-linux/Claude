-- READ-ONLY check: compares p21_set_counter against real MAX(uid) for every alert-related table in P21 Prod.
-- Does NOT advance any counter. Run this first to confirm drift before applying any fix.
-- alert_notification has no physical table in this environment -- not checked.

USE [P21]; -- confirm this is the correct Prod database name before running
GO
...
BEGIN
    ...
    ELSE IF @current_counter < @real_max
        PRINT '  DRIFT DETECTED -- counter is behind by ' + CAST(@real_max - @current_counter AS VARCHAR) + ' (NOT fixed by this script)';
    ELSE
        PRINT '  OK';
