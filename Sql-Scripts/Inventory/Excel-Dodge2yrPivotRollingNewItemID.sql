...
set @query = '	
	select *
	from (
		select m.item_desc,floor(ISNULL(qty_shipped,0))[qty_shipped],(left(datename(month,invoice_date),3) + '' '' + left(datename(YEAR,invoice_date),4))[MonThYearShipped]
		from invoice_hdr h
		join invoice_line il
		on h.invoice_no = il.invoice_no
		join inv_mast m
		on il.inv_mast_uid = m.inv_mast_uid
		where Year(invoice_date) > 2018 and m.item_desc between ''D-000000'' and ''D-999999''
		)
		as s
		PIVOT
		(
