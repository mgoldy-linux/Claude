...
getVesselNumbers(Vinv_mast_uid,vessel_location_id,qty_on_vessel,solve_po_no,external_po_no)
as
(
	select inv_mast_uid,location_id,sum(v.container_qty_received),ph.po_no,ph.external_po_no
	from dbo.po_line p
	...
	group by inv_mast_uid,location_id,ph.po_no,ph.external_po_no
)
select item_desc,SIMG,invoice_price,customer_part_number,customer_name,qty_shipped,invoice_date,qty_on_hand,qty_allocated,qty_Available,coalesce((Qty_on_po - Qty_on_vessel),0)[qty_on_PO],coalesce(vessel_location_id,0)[vessel_location_id],coalesce(qty_on_vessel,0)[vessel_on_qty],qty_on_sales_order,solve_po_no,external_po_no
from getSalesOrders gso
left join getVesselNumbers gvn
on gso.inv_mast_uid = gvn.Vinv_mast_uid
where SIMG = '2101080868'
order by simg
