/*
	 Creted by doug for Ralph. The issue is in excel exceeds 10000 records. Recommend limiting by date?
	 and dbo.oe_hdr.date_created > '2019-12-01' OR and dbo.oe_hdr.date_created > '2019-12-01' - needs be on both sides of the where cause


*/
SELECT        dbo.oe_pick_ticket.pick_ticket_no, dbo.oe_pick_ticket.print_date, COALESCE (dbo.p21_view_clippership_return_10004.carrier_name, dbo.address.name) AS c_carrier, 
                         COALESCE (dbo.p21_view_clippership_return_10004.tracking_no, ptshipment.shipment_id, dbo.oe_pick_ticket.tracking_no) AS c_tracking_no, COALESCE (dbo.p21_view_clippership_return_10004.total_charge, 
                         ...
                         THEN 'R' WHEN oe_pick_ticket_ups.sent_to_roadnet_flag = 'P' THEN 'P' ELSE 'N' END AS roadnet_status, dbo.freight_code.freight_desc, dbo.oe_hdr.ship2_name, dbo.oe_hdr.order_date, dbo.oe_hdr.location_id, 
...
WHERE        (NOT (dbo.oe_pick_ticket.delete_flag = 'Y')) AND (NOT (dbo.oe_pick_ticket.auxiliary = 'Y'))  OR   (NOT (dbo.oe_pick_ticket.pick_and_hold = 'Y')) 