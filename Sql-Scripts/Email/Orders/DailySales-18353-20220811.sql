/*
	...
	08/10/2020 - add new SR per olivia's email
	...
	02/01/2021 - fixed null approved orders
	10/20/2021 - change inc to Corp
	08/11/22 - add loftus@bearingslimited.com
*/
Use P21;
...
-- test for orders
select @Acountorders = Count(Distinct order_no) from [dbo].[Daily_Orders_VW] where salesrep_id = 18353 and approved = 'Y'
select @Ucountorders = Count(Distinct order_no) from [dbo].[Daily_Orders_VW] where salesrep_id = 18353 and approved = 'N'

select @Asumorders = SUM(extended_price) from Daily_Orders_VW where  salesrep_id = 18353 and approved = 'Y'
select @Usumorders = SUM(extended_price) from Daily_Orders_VW where  salesrep_id = 18353 and approved = 'N'
...
if @Asumorders is null
begin
	set @Asumorders = 0
end
else
begin
	set @Asumorders = @Asumorders
end
...
(
	...
	from [dbo].[Daily_Orders_VW] where salesrep_id = 18353
	)
as x;

if @salesRepName = ''
begin
set @salesRepName = ' TPW Corp. (18353) '
end
else 
begin
...
Set @AtableOrders =
	N'<h3><font color="#008000">Previous 24 hrs Approved Orders </font></h3>' +
	N'<table border="1">' +
	...
	N'<th>Approved</th><th>Item No.</th><th>Item Description</th><th>PL</th><th>QTY</th><th>EXT.AMST.$</th></tr>' +
			...
			from Daily_Orders_VW where salesrep_id = 18353 and extended_price != 0 and approved = 'Y'
			order by ZipCode
			for XML Path('tr'), Type
			) AS NVARCHAR(MAX)) + 
			N'</table>'
Set @UtableOrders =
	...
	N'<th>Approved</th><th>Item No.</th><th>Item Description</th><th>PL</th><th>QTY</th><th>EXT.AMST.$</th></tr>' +
			cast ((select "td/@align" = 'left',CompanyName,		'',
						  ...
						  "td/@align" = 'center',qty_ordered,	'',
						  "td/@align" = 'center',extended_price						  
			from Daily_Orders_VW where salesrep_id = 18353 and extended_price != 0 and approved = 'N'
			order by ZipCode
			for XML Path('tr'), Type
			) AS NVARCHAR(MAX)) + 
			N'</table>'
...
if @AtableOrders is null
begin
	set @AtableOrders = ''
End
else
begin 
	set @AtableOrders =  @AtableOrders
end
...
Waitfor delay '00:00:05'
 ...
 EXEC msdb.dbo.sp_send_dbmail
      ...
      @recipients = 'info@tpwcorp.com',
	  @copy_recipients = 'gdib@solveindustrial.com;loftus@bearingslimited.com',
