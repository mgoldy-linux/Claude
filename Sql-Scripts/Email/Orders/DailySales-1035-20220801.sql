/*
	...
	08/01/2022 - add mmaggio@bearingslimited.com
*/
Use P21;
...
-- test for orders
select @Acountorders = Count(Distinct order_no) from [dbo].[Daily_Orders_VW] where salesrep_id = 1035 and approved = 'Y'
select @Ucountorders = Count(Distinct order_no) from [dbo].[Daily_Orders_VW] where salesrep_id = 1035 and approved = 'N'

select @Asumorders = SUM(extended_price) from Daily_Orders_VW where  salesrep_id = 1035 and approved = 'Y'
select @Usumorders = SUM(extended_price) from Daily_Orders_VW where  salesrep_id = 1035 and approved = 'N'
...
(
	...
	from [dbo].[Daily_Orders_VW] where salesrep_id = 1035
	)
as x;

if @salesRepName = ''
begin
set @salesRepName = ' NTS (Sales Rep) (1035) '
end
else 
begin
...
Set @AtableOrders =
	...
	N'<th>Approved</th><th>Item No.</th><th>Item Description</th><th>PL</th><th>QTY</th><th>EXT.AMST.$</th></tr>' +
			...
			from Daily_Orders_VW where salesrep_id = 1035 and extended_price != 0 and approved = 'Y'
			order by ZipCode
			for XML Path('tr'), Type
			) AS NVARCHAR(MAX)) + 
			N'</table>'
Set @UtableOrders =
	...
	N'<th>Approved</th><th>Item No.</th><th>Item Description</th><th>PL</th><th>QTY</th><th>EXT.AMST.$</th></tr>' +
			cast ((select "td/@align" = 'left',CompanyName,		'',
						  ...
						  "td/@align" = 'center',qty_ordered,	'',
						  "td/@align" = 'center',extended_price						  
			from Daily_Orders_VW where salesrep_id = 1035 and extended_price != 0 and approved = 'N'
			order by ZipCode
			for XML Path('tr'), Type
			) AS NVARCHAR(MAX)) + 
			N'</table>'
