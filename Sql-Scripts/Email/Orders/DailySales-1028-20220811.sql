/*
	...
	*** make sure  province vs state
	09/17/2020 - change zip to postal
...
-- test for orders
select @Acountorders = Count(Distinct order_no) from [dbo].[Daily_Orders_VW] where salesrep_id = 1028 and approved = 'Y'
select @Ucountorders = Count(Distinct order_no) from [dbo].[Daily_Orders_VW] where salesrep_id = 1028 and approved = 'N'

select @Asumorders = SUM(extended_price) from Daily_Orders_VW where  salesrep_id = 1028 and approved = 'Y'
select @Usumorders = SUM(extended_price) from Daily_Orders_VW where  salesrep_id = 1028 and approved = 'N'
...
(
	...
	from [dbo].[Daily_Orders_VW] where salesrep_id = 1028
	)
as x;
...
Set @AtableOrders =
	N'<h3><font color="#008000">Approved Orders from previous 24 hrs</font></h3>' +
	N'<table border="1">' +
	N'<tr align="center"><th>Customer''s Name</th><th>Ship to Name</th>' +
	N'<th>Postal Code</th><th>City, Province</th><th>Order Number</th><th>Order Date</th><th>PO Number</th>' +
	N'<th>Approved</th><th>Item No.</th><th>Item Description</th><th>PL</th><th>QTY</th><th>EXT.AMST.$</th></tr>' +
			...
			from Daily_Orders_VW where salesrep_id = 1028 and extended_price != 0 and approved = 'Y'
			order by ZipCode
			for XML Path('tr'), Type
			) AS NVARCHAR(MAX)) + 
			N'</table>'
Set @UtableOrders =
	...
	N'<th>Approved</th><th>Item No.</th><th>Item Description</th><th>PL</th><th>QTY</th><th>EXT.AMST.$</th></tr>' +
			cast ((select "td/@align" = 'left',CompanyName,		'',
						  ...
						  "td/@align" = 'center',qty_ordered,	'',
						  "td/@align" = 'center',extended_price						  
			from Daily_Orders_VW where salesrep_id = 1028 and extended_price != 0 and approved = 'N'
			order by ZipCode
			for XML Path('tr'), Type
			) AS NVARCHAR(MAX)) + 
			N'</table>'
