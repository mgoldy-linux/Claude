/*
	...
	12/22/2020 - no orders cleanup
	08/11/2022 - add loftus@bearingslimited.com
*/
Use P21;
...
-- test for orders
select @Acountorders = Count(Distinct order_no) from [dbo].[Daily_Orders_VW] where salesrep_id = 1025 and approved = 'Y'
select @Ucountorders = Count(Distinct order_no) from [dbo].[Daily_Orders_VW] where salesrep_id = 1025 and approved = 'N'

select @Asumorders = SUM(extended_price) from Daily_Orders_VW where  salesrep_id = 1025 and approved = 'Y'
select @Usumorders = SUM(extended_price) from Daily_Orders_VW where  salesrep_id = 1025 and approved = 'N'
...
(
	...
	from [dbo].[Daily_Orders_VW] where salesrep_id = 1025
	)
as x;

if @salesRepName = ''
begin
set @salesRepName = ' Simpson & Associates (1025) '
end
else 
begin
...
Set @AtableOrders =
	N'<h4><font color="#008000">Approved Orders from previous 24 hrs</font></h4>' +
	N'<table border="1">' +
	...
	N'<th>Approved</th><th>Item No.</th><th>Item Description</th><th>PL</th><th>QTY</th><th>EXT.AMST.$</th></tr>' +
			...
			from Daily_Orders_VW where salesrep_id = 1025 and extended_price != 0 and approved = 'Y'
			order by ZipCode
			for XML Path('tr'), Type
			) AS NVARCHAR(MAX)) + 
			N'</table>'
...
Set @UtableOrders =
	...
	N'<th>Approved</th><th>Item No.</th><th>Item Description</th><th>PL</th><th>QTY</th><th>EXT.AMST.$</th></tr>' +
			cast ((select "td/@align" = 'left',CompanyName,		'',
						  ...
						  "td/@align" = 'center',qty_ordered,	'',
						  "td/@align" = 'center',extended_price						  
			from Daily_Orders_VW where salesrep_id = 1025 and extended_price != 0 and approved = 'N'
			order by ZipCode
			for XML Path('tr'), Type
			) AS NVARCHAR(MAX)) + 
			N'</table>'
