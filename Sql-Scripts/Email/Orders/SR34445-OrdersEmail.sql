...
-- test for orders
select @Acountorders = Count(Distinct order_no) from [dbo].[Daily_Orders_VW] where salesrep_id = 34445 and approved = 'Y'
select @Ucountorders = Count(Distinct order_no) from [dbo].[Daily_Orders_VW] where salesrep_id = 34445 and approved = 'N'

select @Asumorders = SUM(extended_price) from Daily_Orders_VW where  salesrep_id = 34445 and approved = 'Y'
select @Usumorders = SUM(extended_price) from Daily_Orders_VW where  salesrep_id = 34445 and approved = 'N'
...
(
	...
	from [dbo].[Daily_Orders_VW] where salesrep_id = 34445
	)
as x;

if @salesRepName = ''
begin
set @salesRepName = ' Central CPTS (34445) '
end
else 
begin
...
Set @AtableOrders =
	...
	N'<th>Zip Code</th><th>City, State</th><th>Order Number</th><th>Branch ID</th><th>Order Date</th><th>PO Number</th>' +
	N'<th>Approved</th><th>Item No.</th><th>Item Description</th><th>PL</th><th>QTY</th><th>EXT.AMST.$</th></tr>' +
			...
			from Daily_Orders_VW where salesrep_id = 34445 and extended_price != 0 and approved = 'Y'
			order by ZipCode
			for XML Path('tr'), Type
			) AS NVARCHAR(MAX)) + 
			N'</table>'
...
Set @UtableOrders =
	...
	N'<th>Approved</th><th>Item No.</th><th>Item Description</th><th>PL</th><th>QTY</th><th>EXT.AMST.$</th></tr>' +
			cast ((select "td/@align" = 'left',CompanyName,		'',
						  ...
						  "td/@align" = 'center',qty_ordered,	'',
						  "td/@align" = 'center',extended_price						  
			from Daily_Orders_VW where salesrep_id = 34445 and extended_price != 0 and approved = 'N'
			order by ZipCode
			for XML Path('tr'), Type
			) AS NVARCHAR(MAX)) + 
			N'</table>'
...
Waitfor delay '00:00:05'
 ...
 EXEC msdb.dbo.sp_send_dbmail
      ...
      @recipients = 'vcockerham@c-pts.com ',
      @copy_recipients = 'rlinke@solveindustrial.com;bharpur@bearingslimited.com',
