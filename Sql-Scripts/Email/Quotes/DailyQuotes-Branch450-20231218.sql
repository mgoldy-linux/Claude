...
-- test for Quotes
select @countQuotes = Count(Distinct order_no) from [dbo].[Daily_Quotes_VW] where BranchID = 450

select @sumSRQuotes = SUM(extended_price) from Daily_Quotes_VW where  BranchID = 450
...
set @salesRepName = ' Branch 450 '
/*
Select @salesRepName += x.sr 
From
(
	Select Distinct (BranchID) as sr
	from [dbo].[Daily_Quotes_VW] where BranchID = 450
	)
as x;
...
Set @tableQuotes =
	...
	N'<th>Item Description</th><th>PL</th><th>QTY</th><th>EXT.AMST.$</th></tr>' +
		cast ((select "td/@align" = 'center',City + ', ' + SState,	'',
			...
			from Daily_Quotes_VW where BranchID = 450 and extended_price != 0
			order by ZipCode
			for XML Path('tr'), Type
			) AS NVARCHAR(MAX)) + 
			N'</table>'
...
set @Alltables = @tableHighLevel +
 ...
 EXEC msdb.dbo.sp_send_dbmail
      ...
      @recipients = 'james.gouge@solveindustrial.com;kspeicher@c-pts.com; lshields@c-pts.com; jivers@c-pts.com.',
