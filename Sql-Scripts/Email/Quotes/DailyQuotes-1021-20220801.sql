/*
	...
	04/06/2021 - change CC to mmoonan@solveindustrial.com;lmitchell@solveindustrial.com
	06/11/2021 - add email address to table
	01/31/2022 - add kenny@nobisindustrial.com to cc, it to solveindustrial
	02/04/2022 - change importance to normal
	07/07/2022 - add George per email
...
-- test for Quotes
select @countQuotes = Count(Distinct order_no) from [dbo].[Daily_Quotes_VW] where salesrep_id = 1021

select @sumSRQuotes = SUM(extended_price) from Daily_Quotes_VW where  salesrep_id = 1021
...
(
	...
	from [dbo].[Daily_Quotes_VW] where salesrep_id = 1021
	)
as x;
...
Set @tableQuotes =
	...
	N'<th>Item Description</th><th>PL</th><th>QTY</th><th>EXT.AMST.$</th></tr>' +
		cast ((select "td/@align" = 'center',City + ', ' + SState,	'',
			...
			from Daily_Quotes_VW where salesrep_id = 1021 and extended_price != 0
			order by ZipCode
			for XML Path('tr'), Type
			) AS NVARCHAR(MAX)) + 
			N'</table>'
...
set @Alltables = @tableHighLevel +
 ...
 EXEC msdb.dbo.sp_send_dbmail
      ...
      @copy_recipients = 'gdib@solveindustrial.com;mmoonan@solveindustrial.com;lmitchell@solveindustrial.com;kenny@nobisindustrial.com;skuhn@bearingslimited.com',
