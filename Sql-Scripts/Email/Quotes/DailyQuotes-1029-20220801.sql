/*
	...
	01/31/2020 - special format per customer request
	02/03/2020 - remove george, $0 lines - per email, change alignment to center -- not working?
	...
	08/01/2022 - add Scott Kuhn <skuhn@bearingslimited.com> , change importance to normal
...
-- test for Quotes
select @countQuotes = Count(Distinct order_no) from [dbo].[Daily_Quotes_VW] where salesrep_id = 1029

select @sumSRQuotes = SUM(extended_price) from Daily_Quotes_VW where  salesrep_id = 1029
...
(
	...
	from [dbo].[Daily_Quotes_VW] where salesrep_id = 1029
	)
as x;
...
Set @tableQuotes =
	...
	N'<table border="1">' +
	N'<tr align="center"><th>Customer''s Name</th><th>Customer Contact</th><th>email</th><th>Cust, Phone #</th>' +
	N'<th>City, State</th><th>Order Number</th><th>Inside Sales</th><th>Branch</th><th>Item No.</th>' +
	N'<th>Item Description</th><th>PL</th><th>QTY</th><th>EXT.AMST.$</th></tr>' +
		cast ((select"td/@align" = 'center',CompanyName,		'',
			...
			from Daily_Quotes_VW where salesrep_id = 1029 and extended_price != 0
			order by ZipCode
			for XML Path('tr'), Type
			) AS NVARCHAR(MAX)) + 
			N'</table>'
