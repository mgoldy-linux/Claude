/*
	...
	02/04/2021 - fixed incorect SR for no quotes
	03/16/2021 - fixed @copy_recipients = 'gdib@solveindustrial.com'
	06/11/2021 - add email address to table
	08/01/2022 - add Scott Kuhn <skuhn@bearingslimited.com> , change importance to normal & reply email
...
-- test for Quotes
select @countQuotes = Count(Distinct order_no) from [dbo].[Daily_Quotes_VW] where salesrep_id = 1035

select @sumSRQuotes = SUM(extended_price) from Daily_Quotes_VW where  salesrep_id = 1035
...
(
	...
	from [dbo].[Daily_Quotes_VW] where salesrep_id = 1035
	)
as x;
...
Set @tableQuotes =
	...
	N'<th>Item Description</th><th>PL</th><th>QTY</th><th>EXT.AMST.$</th></tr>' +
		cast ((select		 "td/@align" = 'center',City + ', ' + SState,	'',
			...
			from Daily_Quotes_VW where salesrep_id = 1035 and extended_price != 0
			order by ZipCode
			for XML Path('tr'), Type
			) AS NVARCHAR(MAX)) + 
			N'</table>'
