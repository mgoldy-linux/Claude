...
-- test for Quotes
select @countQuotes = Count(Distinct order_no) from [dbo].[Daily_Quotes_VW] where salesrep_id = 18353

select @sumSRQuotes = SUM(extended_price) from Daily_Quotes_VW where  salesrep_id = 18353
...
(
	...
	from [dbo].[Daily_Quotes_VW] where salesrep_id = 18353
	)
as x;
...
Set @tableQuotes =
	...
	N'<th>Item Description</th><th>PL</th><th>QTY</th><th>EXT.AMST.$</th></tr>' +
		cast ((select "td/@align" = 'center',City + ', ' + SState,	'',
			...
			from Daily_Quotes_VW where salesrep_id = 18353 and extended_price != 0
			order by ZipCode
			for XML Path('tr'), Type
			) AS NVARCHAR(MAX)) + 
			N'</table>'
