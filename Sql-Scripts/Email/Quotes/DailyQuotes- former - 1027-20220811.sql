/*
	...
	07/08/2022 - remove rauschco7@gmail.com
...
-- test for Quotes
select @countQuotes = Count(Distinct order_no) from [dbo].[Daily_Quotes_VW] where salesrep_id = 1027

select @sumSRQuotes = SUM(extended_price) from Daily_Quotes_VW where  salesrep_id = 1027
...
Select @Subject = 'Daily Quotes Summary for Former Rausch Sales (1027) Territory for ' + @fullDate
...
Set @tableQuotes =
	...
	N'<th>Item Description</th><th>PL</th><th>QTY</th><th>EXT.AMST.$</th></tr>' +
		cast ((select "td/@align" = 'center',City + ', ' + SState,	'',
			...
			from Daily_Quotes_VW where salesrep_id = 1027 and extended_price != 0
			order by ZipCode
			for XML Path('tr'), Type
			) AS NVARCHAR(MAX)) + 
			N'</table>'
...
set @Alltables = @tableHighLevel +
 ...
 EXEC msdb.dbo.sp_send_dbmail
      ...
      @recipients = 'gdib@solveindustrial.com',
      @copy_recipients = 'loftus@bearingslimited.com',
