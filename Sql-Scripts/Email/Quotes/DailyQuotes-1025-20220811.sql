/*
	...
	06/25/2020 - change shutch@solveindustrial.com to gdib@ptintl
...
-- test for Quotes
select @countQuotes = Count(Distinct order_no) from [dbo].[Daily_Quotes_VW] where salesrep_id = 1025

select @sumSRQuotes = SUM(extended_price) from Daily_Quotes_VW where  salesrep_id = 1025
...
(
	...
	from [dbo].[Daily_Quotes_VW] where salesrep_id = 1025
	)
as x;
...
Set @tableQuotes =
	...
	N'<th>Item Description</th><th>PL</th><th>QTY</th><th>EXT.AMST.$</th></tr>' +
		cast ((select "td/@align" = 'center',City + ', ' + SState,	'',
			...
			from Daily_Quotes_VW where salesrep_id = 1025 and extended_price != 0
			order by ZipCode
			for XML Path('tr'), Type
			) AS NVARCHAR(MAX)) + 
			N'</table>'
