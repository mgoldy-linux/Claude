/*
	05/21/2020 - test email Open quote  Monday Morning at 6 AM
	 @recipients = Sales@reliable-pt.com, CC = dhampton@ptintl.com 
	 File names: Reliable-Pt_1024_Open_Quotes_YYYYMMDD.xlxs
...
declare @Qcount1024 as int,
  ...
  -- set subject Name 
 set @SubName = 'Reliable-Pt'

 -- test for open qoutes for 1024
select @Qcount1024 = COUNT(*) from d_Open_Quote_Summary where SalesRepName like '%24%'
select @Opcount1024 = COUNT(*) from d_Open_Orders_Summary where SalesRepName like '%24%'
...
set @OrderTable = 
	...
	N'<tr align="center"><th>Sales Rep Name</th><th>Number of Open Orders</th><th>Total Value</th></tr>' +
		cast ((select "td/@align" = 'center',SalesRepName,	'',
			...
			from d_Open_Orders_Summary where SalesRepName like '%24%' 
...
set @QuoteTable = 
	...
	N'<tr align="center"><th>Sales Rep Name</th><th>Number of Open Quotes</th><th>Total Value</th></tr>' +
		cast ((select "td/@align" = 'center',SalesRepName,	'',
					  ...
					  "td/@align" = 'center',TotalValue
			from d_Open_Quote_Summary where SalesRepName like '%24%' 
			order by TotalValue desc
			for XML Path('tr'), Type
			) AS NVARCHAR(MAX)) + 
			N'</table>'

if @Qcount1024 >= 1 and @Opcount1024 >= 1
Begin
	set @fileNames = 'C:\SQL_Out\Reliable-Pt_1024_Open_Orders_Quotes_' + @sortDate + '.xlsx' 
End
else if  @Qcount1024 >= 1 and @Opcount1024 = 0
Begin 
	set @fileNames = 'C:\SQL_Out\Reliable-Pt_1024_Open_Orders_Quotes_' + @sortDate + '.xlsx' 
	set @OrderTable =  ''
End
else if  @Qcount1024 = 0 and @Opcount1024 >= 1
Begin 
	set @fileNames = 'C:\SQL_Out\Reliable-Pt_1024_Open_Orders_Quotes_' + @sortDate + '.xlsx' 
	set @QuoteTable =  ''
End
...
EXEC msdb.dbo.sp_send_dbmail
      ...
      --@recipients = 'Sales@reliable-pt.com',
