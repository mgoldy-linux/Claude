/*
	05/21/2020 - test email Open quote  Monday Morning at 6:17 AM
	 @recipients = sales@nobisindustrial.com, CC = dhampton@ptintl.com 
	 File names: Nobis-Industrial_1021_Open_Quotes_YYYYMMDD.xlxs
...
select @Qcount1021 = COUNT(*) from d_Open_Quote_Summary where SalesRepName like '%21%'
select @Opcount1021 = COUNT(*) from d_Open_Orders_Summary where SalesRepName like '%21%'
...
set @OrderTable = 
	...
	N'<tr align="center"><th>Sales Rep Name</th><th>Number of Open Orders</th><th>Total Value</th></tr>' +
		cast ((select "td/@align" = 'center',SalesRepName,	'',
			...
			from d_Open_Orders_Summary where SalesRepName like '%21%' 
...
set @QuoteTable = 
	...
	N'<tr align="center"><th>Sales Rep Name</th><th>Number of Open Quotes</th><th>Total Value</th></tr>' +
		cast ((select "td/@align" = 'center',SalesRepName,	'',
					  ...
					  "td/@align" = 'center',TotalValue
			from d_Open_Quote_Summary where SalesRepName like '%21%' 
			order by TotalValue desc
			for XML Path('tr'), Type
			) AS NVARCHAR(MAX)) + 
			N'</table>'
