/*
	05/21/2020 - test email Open quote  Monday Morning at 6:17 AM
	 @recipients = don@propowerreps.com;ryan@propowerreps.com, CC = shutch@ptintl.com 
	 File names: Power-Associates_1029_Open_Quotes_YYYYMMDD.xlxs
	06/29/2020 - change shutch@ptintl.com to gdib@ptintl
*/
declare @Qcount1029 as int,
		@Opcount1029 as int,
		@dayNum as varchar(3),
 ...
 -- test for open qoutes for 1029
select @Qcount1029 = COUNT(*) from d_Open_Quote_Summary where SalesRepName like '%29%'
select @Opcount1029 = COUNT(*) from d_Open_Orders_Summary where SalesRepName like '%29%'


 -- set subject Name 
 set @SubName = 'Power-Associates'
...
set @OrderTable = 
	...
	N'<tr align="center"><th>Sales Rep Name</th><th>Number of Open Orders</th><th>Total Value</th></tr>' +
		cast ((select "td/@align" = 'center',SalesRepName,	'',
			...
			from d_Open_Orders_Summary where SalesRepName like '%29%' 
...
set @QuoteTable = 
	...
	N'<tr align="center"><th>Sales Rep Name</th><th>Number of Open Quotes</th><th>Total Value</th></tr>' +
		cast ((select "td/@align" = 'center',SalesRepName,	'',
					  ...
					  "td/@align" = 'center',TotalValue
			from d_Open_Quote_Summary where SalesRepName like '%29%' 
			order by TotalValue desc
			for XML Path('tr'), Type
			) AS NVARCHAR(MAX)) + 
			N'</table>'

if @Qcount1029 >= 1 and @Opcount1029 >= 1
Begin
	set @fileNames = 'C:\SQL_Out\Power-Associates_1029_Open_Orders_Quotes_' + @sortDate + '.xlsx' 
End
else if  @Qcount1029 >= 1 and @Opcount1029 = 0
Begin 
	set @fileNames = 'C:\SQL_Out\Power-Associates_1029_Open_Orders_Quotes_' + @sortDate + '.xlsx' 
	set @OrderTable =  ''
End
else if  @Qcount1029 = 0 and @Opcount1029 >= 1
Begin 
	set @fileNames = 'C:\SQL_Out\Power-Associates_1029_Open_Orders_Quotes_' + @sortDate + '.xlsx' 
	set @QuoteTable =  ''
End
