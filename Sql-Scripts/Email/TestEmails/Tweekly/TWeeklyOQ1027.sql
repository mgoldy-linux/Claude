/*
	05/21/2020 - test email Open quote  Monday Morning at 6 AM
	 @recipients = rauschco7@gmail.com, CC = shutch@ptintl.com 
	 File names: Rausch-Sales_1027_Open_Quotes_YYYYMMDD.xlxs
...
-- set subject Name 
set @SubName = 'Rausch-Sales'

-- test for open qoutes for 1027
select @Qcount1027 = COUNT(*) from d_Open_Quote_Summary where SalesRepName like '%27%'
select @Opcount1027 = COUNT(*) from d_Open_Orders_Summary where SalesRepName like '%27%'
...
set @OrderTable = 
	...
	N'<tr align="center"><th>Sales Rep Name</th><th>Number of Open Orders</th><th>Total Value</th></tr>' +
		cast ((select "td/@align" = 'center',SalesRepName,	'',
			...
			from d_Open_Orders_Summary where SalesRepName like '%27%' 
...
set @QuoteTable = 
	...
	N'<tr align="center"><th>Sales Rep Name</th><th>Number of Open Quotes</th><th>Total Value</th></tr>' +
		cast ((select "td/@align" = 'center',SalesRepName,	'',
					  ...
					  "td/@align" = 'center',TotalValue
			from d_Open_Quote_Summary where SalesRepName like '%27%' 
			order by TotalValue desc
			for XML Path('tr'), Type
			) AS NVARCHAR(MAX)) + 
			N'</table>'

if @Qcount1027 >= 1 and @Opcount1027 >= 1
Begin
	set @fileNames = 'C:\SQL_Out\Rausch-Sales_1027_Open_Orders_Quotes_' + @sortDate + '.xlsx' 
End
else if  @Qcount1027 >= 1 and @Opcount1027 = 0
Begin 
	set @fileNames = 'C:\SQL_Out\Rausch-Sales_1027_Open_Orders_Quotes_' + @sortDate + '.xlsx' 
	set @OrderTable =  ''
End
else if  @Qcount1027 = 0 and @Opcount1027 >= 1
Begin 
	set @fileNames = 'C:\SQL_Out\Rausch-Sales_1027_Open_Orders_Quotes_' + @sortDate + '.xlsx' 
	set @QuoteTable =  ''
End
