/*
	05/21/2020 - test email Open quote  Monday Morning at 6 AM
	 @recipients = kevin@allied-components.com, CC = rlinke@ptintl.com 
	 File names: Allied-Components_1032_Open_Quotes_YYYYMMDD.xlxs
	 08/21/2020 - add orders to spreadsheet, change file name: Name_sr_id_Open_Orders_Quotes_YYYYMMDD.xlxs, delete two weeks date
*/
declare @Qcount1032 as int,
		@Opcount1032 as int,
		@dayNum as varchar(3),
  ...
  -- set subject Name 
 set @SubName = 'Allied-Components'

 -- test for open qoutes for 1032
select @Qcount1032 = COUNT(*) from d_Open_Quote_Summary where SalesRepName like '%32%'
select @Opcount1032 = COUNT(*) from d_Open_Orders_Summary where SalesRepName like '%32%'
...
set @OrderTable = 
	...
	N'<tr align="center"><th>Sales Rep Name</th><th>Number of Open Orders</th><th>Total Value</th></tr>' +
		cast ((select "td/@align" = 'center',SalesRepName,	'',
			...
			from d_Open_Orders_Summary where SalesRepName like '%32%' 
...
set @QuoteTable = 
	...
	N'<tr align="center"><th>Sales Rep Name</th><th>Number of Open Quotes</th><th>Total Value</th></tr>' +
		cast ((select "td/@align" = 'center',SalesRepName,	'',
					  ...
					  "td/@align" = 'center',TotalValue
			from d_Open_Quote_Summary where SalesRepName like '%32%' 
			order by TotalValue desc
			for XML Path('tr'), Type
			) AS NVARCHAR(MAX)) + 
			N'</table>'

if @Qcount1032 >= 1 and @Opcount1032 >= 1
Begin
	set @fileNames = 'C:\SQL_Out\Allied-Components_1032_Open_Orders_Quotes_' + @sortDate + '.xlsx' 
End
else if  @Qcount1032 >= 1 and @Opcount1032 = 0
Begin 
	set @fileNames = 'C:\SQL_Out\Allied-Components_1032_Open_Orders_Quotes_' + @sortDate + '.xlsx' 
	set @OrderTable =  ''
End
else if  @Qcount1032 = 0 and @Opcount1032 >= 1
Begin 
	set @fileNames = 'C:\SQL_Out\Allied-Components_1032_Open_Orders_Quotes_' + @sortDate + '.xlsx' 
	set @QuoteTable =  ''
End
...
EXEC msdb.dbo.sp_send_dbmail
      ...
      @recipients = 'kevin@allied-components.com',
