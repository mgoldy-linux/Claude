...
-- test for orders
select @Acountorders = Count(Distinct order_no) from [dbo].[Daily_Yesterday_Orders_VW] where approved = 'Y' and extended_price != 0
select @Ucountorders = Count(Distinct order_no) from [dbo].[Daily_Yesterday_Orders_VW] where approved = 'N' and extended_price != 0
-- set text for hdr
set @TAcountorders = FORMAT(@Acountorders, '###,###,###')
set @TUcountorders = FORMAT(@Ucountorders, '###,###,###')
-- set text for sums
select @Asumorders = SUM(extended_price) from Daily_Yesterday_Orders_VW where approved = 'Y'
set @tasumorders = '$' + FORMAT(@Asumorders, '###,###,###.##')
select @Usumorders = SUM(extended_price) from Daily_Yesterday_Orders_VW where approved = 'N'
set @tusumorders = '$' + FORMAT(@Usumorders, '###,###,###.##')

select @dayNum = DATENAME(DAY,dateadd(day,datediff(day,1,GETDATE()),0))
select @monthName = DATENAME(MONTH,dateadd(day,datediff(day,1,GETDATE()),0))
select @yearNum = DATENAME(YEAR,dateadd(day,datediff(day,1,GETDATE()),0))
select @sortMonth = Month(dateadd(day,datediff(day,1,GETDATE()),0))
...
Set @tableHighLevel =
	...
	cast ((select  "td/@align" = 'center',salesrep_id, '',
			...
			from Daily_Yesterday_Summary_Orders_VW
...
Set @UtableOrders =
	...
	N'<th>Item Description</th><th>PL</th><th>QTY</th><th>EXT.AMST.$</th></tr>' +
		cast ((select "td/@align" = 'center',salesrep_id, '',
			...
			from Daily_Yesterday_Orders_VW where extended_price != 0 and approved = 'N'
...
Set @AtableOrders =
	...
	N'<th>Item Description</th><th>PL</th><th>QTY</th><th>EXT.AMST.$</th></tr>' +
		cast ((select "td/@align" = 'center',salesrep_id, '',
					  ...
					  "td/@align" = 'center',qty_ordered,	'',
					  "td/@align" = 'center',extended_price
			from Daily_Yesterday_Orders_VW where extended_price != 0
			order by salesrep_id,CompanyName,order_no desc
			for XML Path('tr'), Type
			) AS NVARCHAR(MAX)) + 
			N'</table>'
