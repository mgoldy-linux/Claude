/*
	05/21/2020 - test email Open quote  Monday Morning at 6:17 AM
	 @recipients = Info@skadrives.com, CC = shutch@ptintl.com 
	 File names: Simpson-Associates_1025_Open_Quotes_YYYYMMDD.xlxs
...
declare @Qcount1025 as int,
  ...
  -- set subject Name 
 set @SubName = 'Simpson-Associates'

 -- test for open qoutes for 1025
select @Qcount1025 = COUNT(*) from d_Open_Quote_Summary where SalesRepName like '%25%'
select @Opcount1025 = COUNT(*) from d_Open_Orders_Summary where SalesRepName like '%25%'
...
set @OrderTable = 
	...
	N'<tr align="center"><th>Sales Rep Name</th><th>Number of Open Orders</th><th>Total Value</th></tr>' +
		cast ((select "td/@align" = 'center',SalesRepName,	'',
			...
			from d_Open_Orders_Summary where SalesRepName like '%25%' 
...
set @QuoteTable = 
	...
	N'<tr align="center"><th>Sales Rep Name</th><th>Number of Open Quotes</th><th>Total Value</th></tr>' +
		cast ((select "td/@align" = 'center',SalesRepName,	'',
					  ...
					  "td/@align" = 'center',TotalValue
			from d_Open_Quote_Summary where SalesRepName like '%25%' 
			order by TotalValue desc
			for XML Path('tr'), Type
			) AS NVARCHAR(MAX)) + 
			N'</table>'

if @Qcount1025 >= 1 and @Opcount1025 >= 1
Begin
	set @fileNames = 'C:\SQL_Out\Simpson-Associates_1025_Open_Orders_Quotes_' + @sortDate + '.xlsx' 
End
else if  @Qcount1025 >= 1 and @Opcount1025 = 0
Begin 
	set @fileNames = 'C:\SQL_Out\Simpson-Associates_1025_Open_Orders_Quotes_' + @sortDate + '.xlsx' 
	set @OrderTable =  ''
End
else if  @Qcount1025 = 0 and @Opcount1025 >= 1
Begin 
	set @fileNames = 'C:\SQL_Out\Simpson-Associates_1025_Open_Orders_Quotes_' + @sortDate + '.xlsx' 
	set @QuoteTable =  ''
End
