declare @Qcount1021 as int,
 ...
 -- set subject Name 
 set @SubName = 'Nobis-Industrial'

 -- test for open qoutes for 1021
select @Qcount1021 = COUNT(*) from d_Open_Quote_Summary where SalesRepName like '%1021%'
select @Opcount1021 = COUNT(*) from d_Open_Orders_Summary where SalesRepName like '%1021%'
...
set @OrderTable = 
	...
	N'<tr align="center"><th>Sales Rep Name</th><th>Number of Open Orders</th><th>Total Value</th></tr>' +
		cast ((select "td/@align" = 'center',SalesRepName,	'',
			...
			from d_Open_Orders_Summary where SalesRepName like '%1021%' 
...
set @QuoteTable = 
	...
	N'<tr align="center"><th>Sales Rep Name</th><th>Number of Open Quotes</th><th>Total Value</th></tr>' +
		cast ((select "td/@align" = 'center',SalesRepName,	'',
					  ...
					  "td/@align" = 'center',TotalValue
			from d_Open_Quote_Summary where SalesRepName like '%1021%' 
			order by TotalValue desc
			for XML Path('tr'), Type
			) AS NVARCHAR(MAX)) + 
			N'</table>'

if @Qcount1021 >= 1 and @Opcount1021 >= 1
Begin
	set @fileNames = 'C:\SQL_Out\Nobis-Industrial_1021_Open_Orders_Quotes_' + @sortDate + '.xlsx' 
End
else if  @Qcount1021 >= 1 and @Opcount1021 = 0
Begin 
	set @fileNames = 'C:\SQL_Out\Nobis-Industrial_1021_Open_Orders_Quotes_' + @sortDate + '.xlsx' 
	set @OrderTable =  ''
End
else if  @Qcount1021 = 0 and @Opcount1021 >= 1
Begin 
	set @fileNames = 'C:\SQL_Out\Nobis-Industrial_1021_Open_Orders_Quotes_' + @sortDate + '.xlsx' 
	set @QuoteTable =  ''
End
...
EXEC msdb.dbo.sp_send_dbmail
      ...
      @copy_recipients = 'george.dib@solveindustrial.com; kenny@nobisindustrial.com; scott.kuhn@solveindustrial.com; ryan.linke@solveindustrial.com; mark.lamb@solveindustrial.com; Beau.Wiley@solveindustrial.com',
