/*
	05/21/2020 - test email Open quote  Monday Morning at 6:17 AM
	 @recipients = robert@rgwsalescanada.com, CC = gdib@ptintl.com 
	 File names: RGW-Sales_1028_Open_Quotes_YYYYMMDD.xlxs
	 08/21/2020 - add orders to spreadsheet, change file name: Name_sr_id_Open_Orders_Quotes_YYYYMMDD.xlxs, delete two weeks date
*/
declare @Qcount1028 as int,
		@Opcount1028 as int,
		@dayNum as varchar(3),
 ...
 -- set subject Name 
 set @SubName = 'RGW-Sales'

 -- test for open qoutes for 1028
select @Qcount1028 = COUNT(*) from d_Open_Quote_Summary where SalesRepName like '%28%'
select @Opcount1028 = COUNT(*) from d_Open_Orders_Summary where SalesRepName like '%28%'
...
set @OrderTable = 
	...
	N'<tr align="center"><th>Sales Rep Name</th><th>Number of Open Orders</th><th>Total Value</th></tr>' +
		cast ((select "td/@align" = 'center',SalesRepName,	'',
			...
			from d_Open_Orders_Summary where SalesRepName like '%28%' 
...
set @QuoteTable = 
	...
	N'<tr align="center"><th>Sales Rep Name</th><th>Number of Open Quotes</th><th>Total Value</th></tr>' +
		cast ((select "td/@align" = 'center',SalesRepName,	'',
					  ...
					  "td/@align" = 'center',TotalValue
			from d_Open_Quote_Summary where SalesRepName like '%28%' 
			order by TotalValue desc
			for XML Path('tr'), Type
			) AS NVARCHAR(MAX)) + 
			N'</table>'

if @Qcount1028 >= 1 and @Opcount1028 >= 1
Begin
	set @fileNames = 'C:\SQL_Out\RGW-Sales_1028_Open_Orders_Quotes_' + @sortDate + '.xlsx' 
End
else if  @Qcount1028 >= 1 and @Opcount1028 = 0
Begin 
	set @fileNames = 'C:\SQL_Out\RGW-Sales_1028_Open_Orders_Quotes_' + @sortDate + '.xlsx' 
	set @OrderTable =  ''
End
else if  @Qcount1028 = 0 and @Opcount1028 >= 1
Begin 
	set @fileNames = 'C:\SQL_Out\RGW-Sales_1028_Open_Orders_Quotes_' + @sortDate + '.xlsx' 
	set @QuoteTable =  ''
End
