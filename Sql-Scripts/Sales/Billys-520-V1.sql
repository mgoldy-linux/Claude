...
with getinv_mast_uids(uidSST,customer_name,qty_sold,last_invoice_date,invoice_price,customer_part_number)
as
(
...
where invoice_date between '2022-07-18' and getdate() and sales_location_id in (510, 511, 520) and product_group_id != 'OTHERCHG' and product_group_id is not null 
...
as
(
	select item_desc,item_id,customer_name,qty_sold,last_invoice_date,g.uidSST,sum(qty_on_hand),sum(qty_allocated),sum((qty_on_hand - qty_allocated)),sum(order_quantity),sum(ilss.qty_on_sales_order),invoice_price,customer_part_number
	from getinv_mast_uids g
	join dbo.inv_mast m
	on g.uidSST = m.inv_mast_uid
	join dbo.inv_loc il
	on g.uidSST = il.inv_mast_uid
	join inv_loc_stock_status ilss
	on g.uidSST= ilss.inv_mast_uid and il.location_id = ilss.location_id
	group by item_desc,item_id,g.uidSST,customer_name,qty_sold,last_invoice_date,invoice_price,customer_part_number
),
