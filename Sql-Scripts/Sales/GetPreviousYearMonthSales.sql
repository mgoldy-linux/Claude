/*
	 ...
	 02/27/2020 - just the previous year for one month
*/
use P21Play;
/*
go

if OBJECT_ID ('dPreviousYear_Month_Sales', 'V') is not null
drop view dPreviousYear_Month_Sales;
go
create view [dbo].[dPreviousYear_Month_Sales] AS
*/
...
getInvoices(invoice_no,customer_id,inv_mast_uid,item_id,extended_price,product_group_id,p,PFYr)
as
(
	select h.invoice_no,customer_id,l.inv_mast_uid,l.item_id,l.extended_price,l.product_group_id,period,year_for_period
	from invoice_hdr h
	join invoice_line l
	on h.invoice_no = l.invoice_no
	where h.period = @Period and year_for_period = @prevFiscalYear
),
joinCustInvc(customer_id,customer_name,product_group_id,total_sales,cs,p,PFYr)
As
(
	select gcn.customer_id,customer_name,product_group_id,SUM(extended_price),cs,p,PFYr
	from getCustNames gcn
	left join getInvoices gi
	on gcn.customer_id = gi.customer_id
	where product_group_id is not null
	group by gcn.customer_id,customer_name,product_group_id,cs,p,PFYr
)
select customer_id,customer_name,jci.product_group_id,pg.product_group_desc,total_sales,cs,p,PFYr
