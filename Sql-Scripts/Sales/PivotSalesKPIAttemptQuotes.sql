...
set @query = '	
	select *
	from (
	select taker[Taker],count(distinct h.order_no)[numberOfQuotes],sum(extended_price)[Total],location_id,(left(datename(month,order_date),3) + '' '' + left(datename(YEAR,order_date),4))[MonThYearShipped]
	from oe_hdr h
	join oe_line l
	on h.order_no = l.order_no
	join users u
	on h.taker = u.id
	where h.date_created between ''2022-01-01'' and ''2024-01-01'' and h.delete_flag = ''N'' and projected_order = ''Y'' and h.cancel_flag = ''N'' and l.delete_flag = ''N'' and l.cancel_flag = ''N'' and location_id in (100,410,420,430,440,450,460,470,510,520)
