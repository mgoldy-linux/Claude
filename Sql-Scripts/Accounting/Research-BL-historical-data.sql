-- research missing BL invoices issue was NUll class_id1
-- need figure out why don't pull before 07/18/2022
...
--where c.customer_id = 54210
where (class_1id not in ('IPTCI','LMS','PTI') or class_1id is null) and order_date between @StartDate and @EndDate and branch_id like '4%' and product_group_id not in ('OTHERCHG','Null') and qty_shipped != 0 and invoice_line_type = 0
--where c.customer_id = 54210
--where class_1id is NULL and invoice_date between @StartDate and @EndDate and branch_id = @Branch

select top 5 *
from invoice_hdr
where branch_id like '41%'

select top 5 bill2_name,  *
from p21_view_ord_ack_hdr
where title = 'ORDER ACKNOWLEDGEMENT' and bill2_name like 'Gra%'
