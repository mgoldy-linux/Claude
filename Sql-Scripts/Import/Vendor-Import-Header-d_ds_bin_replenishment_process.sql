...
FROM bin_replenishment_order 
INNER JOIN bin_replenishment 
...
INNER JOIN bin_zone ON (bin_zone.bin_zone_uid = from_bin.putaway_zone_uid) 
LEFT JOIN	bin_type ON (bin_type.bin_type_uid = from_bin.bin_type_uid) 
...
GROUP BY bin_replenishment.bin_replenishment_uid ) AS drv_source_bins ON (drv_source_bins.bin_replenishment_uid = bin_replenishment.bin_replenishment_uid) 
