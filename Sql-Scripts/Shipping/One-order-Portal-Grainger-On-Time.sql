...
SELECT ih.invoice_no,oh.po_no,c.customer_id,c.customer_name,ih.ship2_name,ih.ship_to_id,ih.order_no,il.line_no,m.item_id[SIMG],m.item_desc,Cast(il.qty_shipped as int)[qty_shipped],oh.location_id[Sales Loc],m.class_id1[Brand],class_id1.class_description,ol.source_loc_id [Ship Loc],opt.location_id[Pick Loc],convert(varchar(10),ol.required_date,120)[required_date],convert(varchar(10),ih.invoice_date,120)[invoice_date],
case
...
end[Days Late/Early],convert(varchar(10),oh.order_date,120)[order_date],opt.print_date,COALESCE(clip.shipped_date,opt.ship_date) c_ship_date,Convert(varchar(12),DATEADD(minute,DATEDIFF(MINUTE,opt.print_date,COALESCE(clip.shipped_date,opt.ship_date)),0),114)[Print Ship Time Diff],''[Ship Request Diff], ''[Print Request], ''[Order Print]
...
WHERE ih.company_no = '1' AND c.customer_id in (54210,54533,113521)  and oh.order_no = 1538243 ---   AND ih.invoice_date >= DATEADD(day, -90, current_timestamp)
order by invoice_no