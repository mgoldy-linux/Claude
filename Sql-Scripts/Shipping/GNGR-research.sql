...
SELECT ih.invoice_no,il.line_no,m.item_id[SIMG],m.item_desc,Cast(il.qty_shipped as int)[qty_shipped],convert(varchar(10), ol.required_date,120)[required_date],convert(varchar(10), ih.invoice_date,120)[invoice_date],DATEDIFF(day,ol.required_date,ih.invoice_date)[Days Late],oh.po_no,ih.order_no,opt.pick_ticket_no,convert(varchar(10),oh.order_date,120)[order_date],oh.location_id[Sales Loc],m.class_id1[Brand],class_id1.class_description,opt.print_date[print_date],opt.location_id[Pick Loc],ol.source_loc_id [Ship Loc],opt.location_id,ol.line_no
...
WHERE oh.order_no = 1538243 and ol.line_no = 5
