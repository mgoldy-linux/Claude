select c.customer_id, a.id as sequence , c.terms_id, a.name as ShipToName,phys_address1 as address1,phys_address2 as address2,phys_city as city,phys_state as state,phys_postal_code as postalcode,
CASE 
  ...
  ELSE ''
END as Country,phys_country,central_phone_number,po_no_required,
CASE currency_id
  WHEN '1' THEN 'USD'
END as CurrencyCode,
CASE 
...
salesrep_id,st.default_carrier_id, st.preferred_location_id,
...
where (c.company_id = '1' and web_enabled_flag = 'Y') -- and c.customer_id = 125604--or (st.default_branch = 300 and a.phys_country = 'CA')
