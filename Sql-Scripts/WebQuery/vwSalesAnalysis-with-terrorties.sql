SELECT        oe_hdr.po_no_append, customer.legacy_id AS customer_legacy_id, mc.manufacturing_class_desc, supplier_ud.legacy_id, supplier_ud.legacy_company, inv_loc.qty_on_hand, inv_loc.qty_allocated, 
                         ...
                         CASE WHEN invoice_line.invoice_line_uid IN
                             (SELECT        invoice_line_uid_parent
                               FROM            Play2.dbo.invoice_line) THEN 0.00 WHEN invoice_line_type IN (929, 928, 930) THEN 0.00 ELSE 1.00 END AS lines_picked, 1 AS line_count, oe_pick_ticket.tracking_no, oe_hdr.source_code_no, 
                         SourceCode.code_description AS source_code, oe_hdr.source_id, CASE WHEN contactsShip.id IN
                             (SELECT DISTINCT sales_manager_id
                               FROM            Play2.dbo.contacts
...
FROM            Play2.dbo.p21_view_invoice_line AS invoice_line LEFT OUTER JOIN
                         Play2.dbo.oe_pick_ticket AS oe_pick_ticket ON oe_pick_ticket.invoice_id_when_shipped = invoice_line.invoice_no LEFT OUTER JOIN
                         Play2.dbo.invoice_hdr AS invoice_hdr ON invoice_hdr.invoice_no = invoice_line.invoice_no LEFT OUTER JOIN
                         Play2.dbo.invoice_hdr_salesrep AS invoice_hdr_salesrep ON invoice_hdr_salesrep.invoice_number = invoice_hdr.invoice_no AND invoice_hdr_salesrep.primary_salesrep = 'y' LEFT OUTER JOIN
                         ...
                         Play2.dbo.contacts AS manager WITH (NOLOCK) ON manager.id = contactsShip.sales_manager_id LEFT OUTER JOIN
                         Play2.dbo.contacts AS contacts ON contacts.id = invoice_hdr_salesrep.salesrep_id LEFT OUTER JOIN
                         Play2.dbo.oe_line AS oe_line ON oe_line.order_no = invoice_line.order_no AND oe_line.line_no = invoice_line.oe_line_number LEFT OUTER JOIN
                         Play2.dbo.inv_loc AS inv_loc ON inv_loc.inv_mast_uid = oe_line.inv_mast_uid AND inv_loc.location_id = oe_line.source_loc_id LEFT OUTER JOIN
                         Play2.dbo.supplier AS supplier ON invoice_line.supplier_id = supplier.supplier_id LEFT OUTER JOIN
                         Play2.dbo.vendor_supplier AS vendor_supplier ON inv_loc.primary_supplier_id = vendor_supplier.supplier_id AND invoice_hdr.company_no = vendor_supplier.company_id AND vendor_supplier.delete_flag = 'n' AND 
                         vendor_supplier.primary_vendor = 'y' LEFT OUTER JOIN
                         Play2.dbo.vendor AS vendor ON vendor.vendor_id = vendor_supplier.vendor_id AND invoice_hdr.company_no = vendor.company_id LEFT OUTER JOIN
                         Play2.dbo.inventory_supplier AS isup ON isup.inv_mast_uid = inv_loc.inv_mast_uid AND isup.supplier_id = inv_loc.primary_supplier_id LEFT OUTER JOIN
                         Play2.dbo.customer AS customer ON invoice_hdr.customer_id = customer.customer_id AND invoice_hdr.company_no = customer.company_id LEFT OUTER JOIN
                         Play2.dbo.code_p21 AS code_p21 ON code_p21.code_no = invoice_line.invoice_line_type LEFT OUTER JOIN
                         Play2.dbo.discount_group AS discount_group ON discount_group.discount_group_id = inv_loc.purchase_discount_group LEFT OUTER JOIN
                         Play2.dbo.tax_group_hdr AS tax_group_hdr ON inv_loc.tax_group_id = tax_group_hdr.tax_group_id AND inv_loc.company_id = tax_group_hdr.company_id LEFT OUTER JOIN
                         Play2.dbo.product_group AS product_group ON COALESCE (invoice_line.product_group_id, oe_line.product_group_id, inv_loc.product_group_id, 'Blank') = product_group.product_group_id AND 
                         invoice_line.company_id = product_group.company_id LEFT OUTER JOIN
                         Play2.dbo.inv_mast AS inv_mast ON invoice_line.inv_mast_uid = inv_mast.inv_mast_uid LEFT OUTER JOIN
                         Play2.dbo.inv_mast_ud AS iud ON iud.inv_mast_uid = inv_mast.inv_mast_uid LEFT OUTER JOIN
                         Play2.dbo.document_types AS document_types ON document_types.document_type_id = invoice_hdr.invoice_type AND document_types.document_id = 'I' LEFT OUTER JOIN
                         Play2.dbo.invoice_class AS invoice_class ON invoice_hdr.invoice_class = invoice_class.invoice_class LEFT OUTER JOIN
                         Play2.dbo.location AS location ON invoice_hdr.sales_location_id = location.location_id AND invoice_hdr.company_no = location.company_id AND location.delete_flag = 'n' LEFT OUTER JOIN
                         Play2.dbo.contacts AS contacts2 ON customer.salesrep_id = contacts2.id LEFT OUTER JOIN
                         Play2.dbo.branch AS branch ON invoice_hdr.branch_id = branch.branch_id AND invoice_hdr.company_no = branch.company_id LEFT OUTER JOIN
                         Play2.dbo.oe_hdr AS oe_hdr ON invoice_hdr.order_no = oe_hdr.order_no LEFT OUTER JOIN
                         Play2.dbo.company AS company ON invoice_hdr.company_no = company.company_id LEFT OUTER JOIN
                         Play2.dbo.class AS class ON customer.class_1id = class.class_id AND class.class_number = 1 AND class.class_type = 'cs' AND class.delete_flag = 'n' LEFT OUTER JOIN
                         Play2.dbo.class AS class2 ON customer.class_2id = class2.class_id AND class2.class_number = 2 AND class2.class_type = 'cs' AND class2.delete_flag = 'n' LEFT OUTER JOIN
                         Play2.dbo.class AS class3 ON customer.class_3id = class3.class_id AND class3.class_number = 3 AND class3.class_type = 'cs' AND class3.delete_flag = 'n' LEFT OUTER JOIN
                         Play2.dbo.class AS class4 ON customer.class_4id = class4.class_id AND class4.class_number = 4 AND class4.class_type = 'cs' AND class4.delete_flag = 'n' LEFT OUTER JOIN
                         Play2.dbo.class AS class5 ON customer.class_5id = class5.class_id AND class5.class_number = 5 AND class5.class_type = 'cs' AND class5.delete_flag = 'n' LEFT OUTER JOIN
                         Play2.dbo.product_group AS product_group2 ON oe_line.product_group_id = product_group2.product_group_id AND oe_line.company_no = product_group2.company_id LEFT OUTER JOIN
                         Play2.dbo.inv_loc AS inv_loc2 ON invoice_hdr.sales_location_id = inv_loc2.location_id AND invoice_line.inv_mast_uid = inv_loc2.inv_mast_uid LEFT OUTER JOIN
                         Play2.dbo.tax_group_hdr AS tax_group_hdr2 ON inv_loc2.tax_group_id = tax_group_hdr2.tax_group_id AND inv_loc2.company_id = tax_group_hdr2.company_id LEFT OUTER JOIN
                         Play2.dbo.discount_group AS discount_group2 ON inv_loc2.purchase_discount_group = discount_group2.discount_group_id LEFT OUTER JOIN
                         Play2.dbo.corp_id AS Corp_ID ON Corp_ID.company_id = invoice_hdr.company_no AND Corp_ID.address_id = invoice_hdr.corp_address_id AND Corp_ID.delete_flag = 'n' LEFT OUTER JOIN
                         Play2.dbo.class AS oe_hdr_class WITH (NOLOCK) ON oe_hdr.class_1id = oe_hdr_class.class_id AND oe_hdr_class.class_number = 1 AND oe_hdr_class.class_type = 'oe' AND oe_hdr_class.delete_flag = 'n' LEFT OUTER JOIN
                         Play2.dbo.class AS oe_hdr_class2 WITH (NOLOCK) ON oe_hdr.class_2id = oe_hdr_class2.class_id AND oe_hdr_class2.class_number = 2 AND oe_hdr_class2.class_type = 'oe' AND 
                         oe_hdr_class2.delete_flag = 'n' LEFT OUTER JOIN
                         Play2.dbo.class AS oe_hdr_class3 WITH (NOLOCK) ON oe_hdr.class_3id = oe_hdr_class3.class_id AND oe_hdr_class3.class_number = 3 AND oe_hdr_class3.class_type = 'oe' AND 
                         oe_hdr_class3.delete_flag = 'n' LEFT OUTER JOIN
                         Play2.dbo.class AS oe_hdr_class4 WITH (NOLOCK) ON oe_hdr.class_4id = oe_hdr_class4.class_id AND oe_hdr_class4.class_number = 4 AND oe_hdr_class4.class_type = 'oe' AND 
                         oe_hdr_class4.delete_flag = 'n' LEFT OUTER JOIN
                         Play2.dbo.class AS oe_hdr_class5 WITH (NOLOCK) ON oe_hdr.class_5id = oe_hdr_class5.class_id AND oe_hdr_class5.class_number = 5 AND oe_hdr_class5.class_type = 'oe' AND 
                         oe_hdr_class5.delete_flag = 'n' LEFT OUTER JOIN
                         Play2.dbo.ship_to AS ship_to WITH (NOLOCK) ON ship_to.ship_to_id = invoice_hdr.ship_to_id AND ship_to.company_id = invoice_hdr.company_no AND ship_to.delete_flag <> 'y' LEFT OUTER JOIN
                         Play2.dbo.tax_group_hdr AS shptax_group_hdr WITH (NOLOCK) ON ship_to.tax_group_id = shptax_group_hdr.tax_group_id AND ship_to.company_id = shptax_group_hdr.company_id AND 
                         shptax_group_hdr.delete_flag = 'n' LEFT OUTER JOIN
                         Play2.dbo.class AS inv_mast_class1 WITH (NOLOCK) ON inv_mast.class_id1 = inv_mast_class1.class_id AND inv_mast_class1.class_number = 1 AND inv_mast_class1.class_type = 'iv' AND 
                         inv_mast_class1.delete_flag = 'n' LEFT OUTER JOIN
                         Play2.dbo.class AS inv_mast_class2 WITH (NOLOCK) ON inv_mast.class_id2 = inv_mast_class2.class_id AND inv_mast_class2.class_number = 2 AND inv_mast_class2.class_type = 'iv' AND 
                         inv_mast_class2.delete_flag = 'n' LEFT OUTER JOIN
                         Play2.dbo.class AS inv_mast_class3 WITH (NOLOCK) ON inv_mast.class_id3 = inv_mast_class3.class_id AND inv_mast_class3.class_number = 3 AND inv_mast_class3.class_type = 'iv' AND 
                         inv_mast_class3.delete_flag = 'n' LEFT OUTER JOIN
                         Play2.dbo.class AS inv_mast_class4 WITH (NOLOCK) ON inv_mast.class_id4 = inv_mast_class4.class_id AND inv_mast_class4.class_number = 4 AND inv_mast_class4.class_type = 'iv' AND 
                         inv_mast_class4.delete_flag = 'n' LEFT OUTER JOIN
                         Play2.dbo.class AS inv_mast_class5 WITH (NOLOCK) ON inv_mast.class_id5 = inv_mast_class5.class_id AND inv_mast_class5.class_number = 5 AND inv_mast_class5.class_type = 'iv' AND 
                         inv_mast_class5.delete_flag = 'n' LEFT OUTER JOIN
                         Play2.dbo.item_uom AS item_uom WITH (NOLOCK) ON item_uom.inv_mast_uid = invoice_line.inv_mast_uid AND item_uom.unit_of_measure = invoice_line.unit_of_measure LEFT OUTER JOIN
                         Play2.dbo.code_p21 AS SourceCode ON oe_hdr.source_code_no = SourceCode.code_no AND SourceCode.row_status_flag = 'A' LEFT OUTER JOIN
                         Play2.dbo.periods AS p ON p.period = invoice_hdr.period AND p.year_for_period = invoice_hdr.year_for_period AND p.company_no = invoice_hdr.company_no LEFT OUTER JOIN
                         Play2.dbo.address AS custAddress WITH (NOLOCK) ON customer.customer_id = custAddress.id LEFT OUTER JOIN
                         Play2.dbo.price_page AS pp WITH (NOLOCK) ON pp.price_page_uid = oe_line.price_page_uid LEFT OUTER JOIN
                         Play2.dbo.code_p21 AS ppc WITH (NOLOCK) ON ppc.code_no = pp.calculation_method_cd LEFT OUTER JOIN
                         ...
                         Play2.dbo.rma_receipt_hdr AS rrh ON rrh.invoice_no = invoice_hdr.invoice_no LEFT OUTER JOIN
                         Play2.dbo.rma_receipt_line AS rrl WITH (NOLOCK) ON rrl.oe_line_uid = oe_line.oe_line_uid AND rrh.rma_receipt_hdr_uid = rrl.rma_receipt_hdr_uid LEFT OUTER JOIN
                         Play2.dbo.reason AS reason WITH (NOLOCK) ON reason.id = rrl.reason_adjustment_id LEFT OUTER JOIN
                         Play2.dbo.inv_loc_stock_status AS ss WITH (NOLOCK) ON ss.inv_mast_uid = invoice_line.inv_mast_uid AND ss.location_id = oe_line.source_loc_id LEFT OUTER JOIN
                         Play2.dbo.assembly_hdr AS as1 ON as1.inv_mast_uid = inv_mast.inv_mast_uid LEFT OUTER JOIN
                         Play2.dbo.inv_mast AS am ON am.inv_mast_uid = as1.inv_mast_uid LEFT OUTER JOIN
                             (SELECT        vessel_receipts_hdr.location_id, po_line.inv_mast_uid, SUM(vessel_receipts_line.container_qty_received) AS total_qty_on_vessel
                               FROM            Play2.dbo.vessel_receipts_line AS vessel_receipts_line WITH (NOLOCK) INNER JOIN
                                                         Play2.dbo.vessel_receipts_hdr AS vessel_receipts_hdr ON vessel_receipts_hdr.vessel_receipts_hdr_uid = vessel_receipts_line.vessel_receipts_hdr_uid LEFT OUTER JOIN
                                                         Play2.dbo.po_line AS po_line WITH (NOLOCK) ON po_line.po_line_uid = vessel_receipts_line.po_line_uid
                         ...
                         Play2.dbo.manufacturing_class AS mc WITH (NOLOCK) ON mc.manufacturing_class_id = isup.manufacturing_class_id LEFT OUTER JOIN
                         Play2.dbo.supplier_ud AS supplier_ud WITH (NOLOCK) ON supplier_ud.supplier_id = inv_loc.primary_supplier_id CROSS JOIN
                             (SELECT DISTINCT year_for_period
                               FROM            Play2.dbo.periods
                               WHERE        (CAST(CAST(GETDATE() AS DATE) AS DATETIME) BETWEEN beginning_date AND ending_date) AND (company_no = 1)) AS currentyear LEFT OUTER JOIN Play2.dbo.territory_x_customer txc on customer.customer_id = txc.customer_id left join
						 Play2.dbo.territory t on txc.territory_uid = t.territory_uid
						 -- LEFT OUTER JOIN P21.dbo.territory_x_customer txc on customer.customer_id = txc.customer_id left join
...
WHERE        (invoice_hdr.approved = 'Y') AND (invoice_hdr.invoice_class <> 'FINANCE') AND (invoice_hdr.invoice_adjustment_type NOT IN ('B', 'T', 'X', 'R', 'W', 'A', 'P', 'Z', 'Y')) AND (invoice_hdr.consolidated <> 'C') AND 
                         (invoice_line.invoice_line_type NOT IN (3)) AND (invoice_hdr.original_document_type <> 'T') AND (invoice_line.item_id NOT IN ('DOWNPAYMENT', 'PREPAYMENT')) AND (invoice_line.qty_shipped > 0.0000 OR
                         ...
                         (invoice_line.invoice_line_type NOT IN (3)) AND (invoice_hdr.original_document_type <> 'T') AND (invoice_line.item_id NOT IN ('DOWNPAYMENT', 'PREPAYMENT')) AND (invoice_line.qty_shipped > 0.0000 OR
                         invoice_line.qty_shipped < 0.0000) AND (invoice_line.item_id <> 'ADVANCE BILL AMOUNT') AND (oe_line.lot_bill = 'Y') AND (invoice_line.invoice_line_uid_parent = 0) AND (invoice_line.invoice_line_type = 4)